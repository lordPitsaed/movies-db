...
function App() {
  ...
  useEffect(() => {
    moviesDbService.getGenresList().then((list) => setGenresList(list.genres));
    moviesDbService
      .createGuestSession()
  ...
  const getRatedMovies = (page: number) => {
    moviesDbService.getRatedMovies(guestId, page).then(({ movies, pages }) => {
  ...
  return (
    <div className='app'>
      {errorBound.isError ? (
        <ErrorAlert error={errorBound.error} />
      ) : (
